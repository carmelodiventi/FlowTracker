...
extern "C" fn did_finish_launching(this: &Object, _: Sel, _: id) {
  ...
  // Wrap in catch_unwind to prevent a panic from crossing the extern "C"
  // boundary on macOS 26 (Tahoe). Log the panic and continue.
  ...
  if let Err(e) = result {
    ...
    eprintln!("[tao-patch] did_finish_launching panic caught: {msg}");
    // Reset in_callback flag and activate the app so the event loop and
    // window work correctly despite the caught panic.
    AppState::recover_from_launch_panic();
  }
