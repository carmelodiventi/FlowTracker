...
impl Handler {
  ...
  fn handle_nonuser_event(&self, wrapper: EventWrapper) {
    if let Some(ref mut callback) = *self.callback.lock().unwrap_or_else(|e| e.into_inner()) {
      match wrapper {
        EventWrapper::StaticEvent(event) => {
          callback.handle_nonuser_event(event, &mut self.control_flow.lock().unwrap_or_else(|e| e.into_inner()))
        }
  ...
  fn handle_user_events(&self) {
    if let Some(ref mut callback) = *self.callback.lock().unwrap_or_else(|e| e.into_inner()) {
      callback.handle_user_events(&mut self.control_flow.lock().unwrap_or_else(|e| e.into_inner()));
    }
  }
...
impl AppState {
  ...
  pub fn launched(app_delegate: &Object) {
    ...
    unsafe {
      ...
      // NOTE: `activateIgnoringOtherApps` was deprecated in macOS 14 and
      // throws NSException on macOS 26 (Tahoe). The replacement `activate()`
      // also throws NSImageCacheException on macOS 26 when running without a
      // bundled .app (no valid dock icon). Skip explicit activation — the
      // WKWebView window will still appear and take focus on its own.
      // ns_app.activate(); // DISABLED for macOS 26 compatibility
  ...
  /// Called from `did_finish_launching` error handler to recover from a panic
  /// mid-way through `launched()`. Resets in_callback state so that the event
  /// loop can continue processing events after the caught panic.
  pub fn recover_from_launch_panic() {
    // If in_callback is stuck as true after a caught panic, reset it so that
    // subsequent event callbacks (wakeup, handle_user_events, etc.) don't
    // short-circuit thinking we're still inside a callback.
    if HANDLER.get_in_callback() {
      HANDLER.set_in_callback(false);
    }
    // Activate the app so its window comes to the foreground.
    unsafe {
      if let Some(mtm) = objc2::MainThreadMarker::new() {
        let ns_app = objc2_app_kit::NSApp(mtm);
        // Wrap in catch_unwind — activate() itself may throw on macOS 26 in
        // some edge cases, but it's safe to ignore if it does.
